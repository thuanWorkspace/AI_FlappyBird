...
//		}
	...
	public GameStatePlay(GameScreen gameScreen) {
		...
		flappAlgo = new FlappAStar(gameScreen.runBFS);
	}
...
//			}
	...
	public void draw(Graphics g) {
		...
		flappAlgo.setRunBFS(gameScreen.runBFS);
		flappAlgo.setRunAstart(gameScreen.astart);

		for (int i = 0; i < flappAlgo.getSpaceNodes().size(); i++) {
			try {
				Node node = flappAlgo.getSpaceNodes().get(i);
				g.drawOval(node.x, node.y, 10, 10);
				node.setX(node.x - 4);
			} catch (Exception e) {
		...
		g.setFont(font2);
		if(flappAlgo.leftRemain!=0) {
			g.drawString(flappAlgo.leftRemain+" unpexloit nodes", flappAlgo.x, flappAlgo.y);
		}
		g.setColor(Color.red);
//		for (Node node : flappAlgo.getNodes()) {
		for (int i = 0; i < flappAlgo.getNodes().size(); i++) {
			try {
				Node node = flappAlgo.getNodes().get(i);
				g.drawOval(node.x, node.y, 10, 10);
				node.setX(node.x - 4);
			} catch (Exception e) {
				// TODO: handle exception
		...
		flappAlgo.setX(flappAlgo.getX()-4);
